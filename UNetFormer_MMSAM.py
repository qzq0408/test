...
from MedSAM.models.ImageEncoder.vit.boundary_utils import boundary_alignment_loss
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        self.sam = sam_model_registry["vit_h"](args,checkpoint='/workspace/qzq/MFNet/checkpoint/sam_vit_h_4b8939.pth')
        ...
        for n, value in self.image_encoder.named_parameters():
            if ("Adapter" not in n and "tune_" not in n and "fourier" not in n
                    and "band" not in n and "mlp_up" not in n and "spatial_gate" not in n):
            # if 'lora_' not in n:
    ...
    def forward(self, x, y, mode='Train', target=None):
        h, w = x.size()[-2:]
        y = torch.unsqueeze(y, dim=1).repeat(1,3,1,1)
        enc_out = self.image_encoder(x, y)
        if isinstance(enc_out, tuple) and len(enc_out) == 3:
            deepx, deepy, edge_aux = enc_out
        else:
            deepx, deepy = enc_out
            edge_aux = None
        ...
        if mode == 'Train' and target is not None and edge_aux is not None:
            bd_loss = boundary_alignment_loss(
                edge_aux["edge_x"],
                edge_aux["edge_y"],
                target,
            )
            if self.args.boundary_loss_weight > 0:
                return x, bd_loss
